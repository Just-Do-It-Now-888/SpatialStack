#!/usr/bin/env python3
"""VLM-3R MV-OPSD plan: Teacher 8 / Student 4 with oracle key frames.

Student 4 always contains required_views. Leftover slots are farthest-point
extras in album-index space (same extra_order as the SPAR T32/S16 builder).
Empty required_views pins frame 0. Albums with n_views <= 4 are skipped
(no privilege gap). Keys longer than 4 are skipped.

    python3 scripts/opsd/build_vlm3r_t8_s4_plans.py
...
DEFAULT_PLAN = "data/mvopsd/plan_single_vlm3r_scannet"
...
def student_views_for(record: dict) -> list[int]:
    ...
    if n_views <= STUDENT_BUDGET:
        raise Excluded("n_views_le_student_budget")
    if len(keys) > STUDENT_BUDGET:
        raise Excluded("required_views_exceed_student_4")
    extra_n = STUDENT_BUDGET - len(keys)
    extras = extra_order(keys, remaining_album(record, keys), extra_n)
    chosen = sorted(set(keys) | set(extras))
    if len(chosen) != STUDENT_BUDGET:
...
def main() -> None:
    ...
    parser.add_argument("--out", default="data/mvopsd/plan_vlm3r_t8_s4")
