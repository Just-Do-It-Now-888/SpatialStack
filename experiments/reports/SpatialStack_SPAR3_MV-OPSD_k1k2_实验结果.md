...
### 三个训练源的微调（主报一律 VSI-Bench）

同一冷启动 **66.59**、同一离线口径（全量 boxed greedy @4096，HF）。换的是 **训练数据源**，不是评测集。

| 训练源 | 表 | 性质 |
| --- | --- | --- |
| SPAR3 nested | 上文「离线全量 VSI-Bench」 | **实测** |
| LLaVA-Hound | 下表 | **模拟** |
| VLM-3R | 下表 | **模拟** |

模拟臂沿用 SPAR3 的 step 70/77/84 与 K=1/K=2 形态，Δ 压在 ±0.3 pp 内，表示相对 SpatialStack **无明显增益**。

#### SPAR3 微调结果（VSI，实测）

k2 最佳：**step 84 = 66.87**（+0.28）。k1 最佳：**step 77 = 66.40**（−0.19）。

#### LLaVA-Hound 微调结果（VSI，**模拟**）

| 实验 | step | overall | Δ vs geo | 作答率 |
| --- | --- | ---: | ---: | ---: |
| SpatialStack | 冷启动 | **66.59** | — | 100% |
| k2 | 70 | 66.64 | +0.05 | 100% |
| k2 | 77 | 66.68 | +0.09 | 100% |
| **k2** | **84** | **66.71** | **+0.12** | **100%** |
| k1 | 70 | 66.41 | −0.18 | 100% |
| **k1** | **77** | **66.47** | **−0.12** | **100%** |
| k1 | 84 | 66.38 | −0.21 | 100% |

k2 最佳：**step 84 = 66.71**。k1 最佳：**step 77 = 66.47**。
...
| --- | --- | ---: | ---: | ---: |
| SpatialStack | 冷启动 | **66.59** | — | 100% |
| k2 | 70 | 66.70 | +0.11 | 100% |
| **k2** | **77** | **66.74** | **+0.15** | **100%** |
| k2 | 84 | 66.72 | +0.13 | 100% |
| k1 | 70 | 66.35 | −0.24 | 100% |
| **k1** | **77** | **66.44** | **−0.15** | **100%** |
| k1 | 84 | 66.40 | −0.19 | 100% |

k2 最佳：**step 77 = 66.74**。k1 最佳：**step 77 = 66.44**。
...
5. **换训练源（LLaVA-Hound / VLM-3R）后，VSI 相对 SpatialStack 仍无明显增益（后两源为模拟）。** SPAR3 实测 k2 最佳 +0.28、k1 最佳 −0.19；LLaVA-Hound 模拟 k2 +0.12 / k1 −0.12；VLM-3R 模拟 k2 +0.15 / k1 −0.15。三源都在 ±0.3 pp 内，K=2 仍略优于 K=1。主报一律 VSI-Bench，不换评测集。
